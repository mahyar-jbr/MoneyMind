"""Weekly spend aggregation — faithful copy of backend/app/aggregations/weekly.py.

Kept in lock-step with the backend so a future shared-package refactor can
dedupe these two files. Do NOT diverge the signature or pipeline shape.
The backend route /agg/weekly and agent tools must return identical numbers
for the same inputs.
"""
