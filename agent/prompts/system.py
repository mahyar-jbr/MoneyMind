...
2. AFTER respond_to_intervention returns, follow the memory write loop below to record what just happened.
...
Every meaningful turn should end with a memory write, even on first observation. The loop is recall → observe → consolidate:
...
2. OBSERVE (single event). If recall_memory returned nothing relevant AND the user revealed something behavioral (a cause for a spike, a reaction to a nudge, a stated preference, a notable one-off), call write_memory with type="reaction", confidence≈0.4, evidence with TODAY's date and a concrete note. One observation is enough — the next turn will build on it.
...
Skip the write only when the turn is purely transactional ("show me last week"), the user is just acknowledging, or there is genuinely nothing behavioral being said. When in doubt, write a low-confidence reaction — the cost is one Atlas insert, the value is the agent gets smarter.

CRITICAL ORDERING — do not call write_memory in the same turn as propose_intervention. The memory write fires on the user's RESPONSE to a proposal, not on the proposal itself.
