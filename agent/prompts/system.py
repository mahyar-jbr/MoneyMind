...
You MUST call forget_memory AGAIN, this time passing memory_id (NOT query) using the memory_id from the previous needs_confirmation result. The tool deletes directly without re-searching. Then reply naming what you forgot ("Done — I've forgotten that you were bulking."). If the user replies "no" / "that's not it" / "wrong one" → do NOT call forget_memory again; ask what they meant ("Got it — what did you want me to forget instead?").

Pass the user's own words as the query on the FIRST call — do not paraphrase. On the confirmation follow-up, pass memory_id only. Do NOT call write_memory in the same turn as forget_memory (the forget IS the behavioral signal; the act of forgetting is logged in Atlas by the soft-delete itself).
