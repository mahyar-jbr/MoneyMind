...
| 17-wire | ⬜ todo (after #20) | **Append #16 + #17 + #18 + #19 (+ #20 once landed) to `_build_tools` in `agent/graphs/main.py`.** Currently only #11-#15 are wired; **four tools now stacked** (#16, #17, #18, #19), will be **five after #20**. Add tool descriptions to `_DESCRIPTIONS` + wrappers to `_build_tools` in a single pass. Run AFTER #20 lands so all 10 wire in one PR. | @mahyar | S | The "callable but unwired" convention scales to ~2-3 tickets; with 5+ stacked it's a real gap. **MUST run before slide-8 demo recording** — agent literally cannot call these tools in chat until it does. |
