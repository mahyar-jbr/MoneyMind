...
| 17-wire | ✅ done (2026-06-02) | **Append #16 + #17 + #18 + #19 + #20 to `_build_tools` in `agent/graphs/main.py`.** Currently only #11-#15 are wired; all five remaining tools are callable but unwired. | @mahyar | S | LANDED. All 10 tools now reachable from chat. 5 new test_graph scenarios; 201 tests green; ruff clean. **Live verification on real Atlas + real Gemini:** asked "How am I doing on food spending this week?" → Gemini picked summarize_week on first attempt → tool fired → GoalSnapshot's `note` field ("Behind by 9pp.") surfaced as natural prose in the reply ("11 percentage points behind schedule"). Slide-8 substrate is mechanically live end-to-end. The forward reference inside query_transactions' description is now truthful. |
