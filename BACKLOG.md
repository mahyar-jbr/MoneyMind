...
| 12a | ⚠️ **9 reports flagged** | **Delete `backend/app/api/chat 2.py` + add `* 2.*` patterns to `.gitignore`.** Byte-identical Finder duplicate, still on disk. #17 dev report literally suggested "the right play is to just `git rm` it on the next backend PR" — giving Kasra the explicit out. | @kasra | S | Mahyar: tell Kasra he can just bundle the delete into whatever backend PR he ships next. |
...
| 17-wire | ⬜ todo | **Append #16 + #17 (and #18+ once landed) to `_build_tools` in `agent/graphs/main.py`.** Currently only #11-#15 are wired into the LangGraph graph; #16 check_goal_pace and #17 propose_intervention are unwired. Add the tool descriptions to `_DESCRIPTIONS` and the wrappers to `_build_tools`. Batch with #18+ to avoid graph churn. | @mahyar | S | The "callable but unwired" convention is fine for a few tickets; with 4+ stacked it becomes a real gap. Run before slide-8 demo recording. |
