...
> **Goal:** all agent tools live (originally scoped at 10; shipped 12 incl. `respond_to_intervention` follow-up #17a and `forget_memory` from V3), weekly cron loop, intervention approval flow. The slide-8 scenario works live.
...
**What shipped (PR #52):**
- NEW [`frontend/components/dashboard/widgets.tsx`](frontend/components/dashboard/widgets.tsx) (~540 lines after the 2026-06-08 scope cut) — 10 widgets: `KpiRow`, `InsightsPanel`, `TrendChart`, `BudgetProgress`, `CategoryBreakdown`, `IncomeExpenses`, `SavingsGoals`, `LargestPurchases`, `DashboardFilters`, plus `CARD` constant + `Panel`/`Kpi`/`Bar`/`DemoTag` primitives. Replaces the deleted [`spend-chart.tsx`](frontend/components/dashboard/spend-chart.tsx), [`category-breakdown.tsx`](frontend/components/dashboard/category-breakdown.tsx), [`stat-cards.tsx`](frontend/components/dashboard/stat-cards.tsx). `UpcomingBills` was shipped in PR #52 then removed 2026-06-08 (see "Demo-readiness scope cut" below).
...
**Demo-readiness scope cut (2026-06-08):**
After a widget-by-widget audit of what's backed by real data vs. `DEMO_` constants, three surfaces were removed because there is no path to making them real without Plaid/bank-linking (which the MongoDB-track pitch explicitly rules out): **Net worth KPI**, **Cash available KPI**, and the **Upcoming bills widget**. The two KPI slots were replaced with `Avg / day` (computed from the selected bucket's spend ÷ days-in-period) and `Top category` (from `rankCategories(selected.byCategory, 1)`). The `DEMO_NET_WORTH`, `DEMO_CASH_AVAILABLE`, `DEMO_BILLS` constants and the `UpcomingBills` export were deleted; `DEMO_BUDGETS` and `DEMO_GOALS` stayed (BudgetProgress will be swapped to real data by V6 below; SavingsGoals by V1). Dashboard grid restructured so SavingsGoals now pairs with LargestPurchases and TransactionsList takes full width. Files: [`widgets.tsx`](frontend/components/dashboard/widgets.tsx), [`page.tsx`](frontend/app/dashboard/page.tsx), [`demo.ts`](frontend/lib/demo.ts). `tsc --noEmit` clean.
...
**Owner:** @aidin (PR #52) + @mahyar (post-merge fixes + 2026-06-08 scope cut)
**Recommendation:** SHIPPED — closes V2. Dashboard now passes the "real finance app" bar for the demo video; every visible KPI/chart is backed by real Atlas data, the two remaining SAMPLE-tagged widgets (BudgetProgress, SavingsGoals) have explicit swap targets in V6 and V1.
