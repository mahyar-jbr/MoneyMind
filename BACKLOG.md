...
| 24  | 🟡 **wip 2026-06-02** Wire MongoDB MCP server into agent       | @mahyar  | M    | Agent self-tunes one query in demo. **Mahyar building tonight.** Critical-path for MongoDB-track wow beat. |
...
| 22a | 🟡 **SHIP TONIGHT** | **Real backend for the intervention loop** — two thin endpoints: `GET /interventions/pending` + `POST /interventions/{id}/respond`. The POST does NOT do the Mongo update directly — it imports `agent.tools.respond_to_intervention` and calls the coroutine (same monorepo pattern as `fetch_agent_weekly_summary`), so #17a's tool stays the single source of truth for the U-verb. JWT-auth per #4a; loopback unaffected. GET uses the `{user_id:1, status:1, proposed_at:-1}` index from #21. **Folds 409 on already-responded, 400 on validation.** | @kasra | S | Mahyar's call: option (a). Slide-8 centerpiece is real end-to-end after this. **Deadline: EOD Jun 3** so Aidin can swap the mock + smoke-test before #26/#27. |
