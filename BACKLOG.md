...
| 23a | ⚠️ **demo-beat at risk** | **Dashboard must render `/inbox`** — Kasra's #21 ships JSON at GET `/inbox` with weekly-summary + reminder messages. Aidin's #23 dashboard shipped without rendering it; Aidin is now on #22 (intervention card). The demo video's "agent posts to inbox" beat has no visible surface. | @aidin | S | Re-flagged twice (#21 + #23 reviews). Mahyar: needs to be Aidin's next ticket after #22, OR cut from the demo script. |
| 23b | ⬜ todo | **`formatWeek` parses dates as local time, backend returns UTC midnight.** Off-by-one possible near TZ boundaries. Demo records in EST/PST so a single take is safe, but multi-take video near midnight could surface drift. One-line `Date(... + 'T00:00:00Z')` fix in `frontend/lib/dashboard.ts`. | @aidin | XS | Surfaced by #23 review. Edge case, but trivial to fix. |
