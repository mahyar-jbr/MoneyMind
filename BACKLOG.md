...
| 16a | ⬜ todo | **Decide: `goals.current_amount` is hand-set or auto-recomputed from `savings.*` transactions.** Today it's hand-set — #16's demo `$set`s it directly to land verdicts. Synthetic CSV has biweekly $115 Emergency Fund Transfer rows that *would* drive it if summed. Decision needed BEFORE #21 cron writes goal docs. Either: (a) keep hand-set, document in data-model.md; (b) define a `savings.<goal_tag>` category convention + recompute helper. | @mahyar | S | Surfaced by #16. Ambiguity is the real risk. |
| 17a | ⬜ todo | **Tool: `respond_to_intervention(intervention_id, user_response)`** — the R+U half of the intervention lifecycle. #17 dev report pre-spec'd the contract: lookup by intervention_id + user_id; set `user_response ∈ {accepted, declined, modified, ignored}`; set `responded_at = datetime.now(UTC)`; flip `status` from `pending` → `responded` (or `ignored`); do NOT touch `related_memory.use_count` (that's #13a's job, fires on accept). Likely demo-critical for slide 8 — without it, the agent proposes but the response never closes the loop. | @mahyar | S | Pre-spec'd in #17 dev report — saves a ticket-writing pass. |
| 17b | ⬜ todo | **Compound index on `interventions`** — `{user_id: 1, status: 1, proposed_at: -1}`. #21 cron + #22 UI both query "pending interventions for user X, most recent first" — without the index they table-scan. | @kasra | S | Surfaced by #17 dev report. Whoever lands #21 or #22 first should add it. |
| 17-wire | 🟡 **THE BOTTLENECK** | **Append #16 + #17 + #18 + #19 + #20 to `_build_tools` in `agent/graphs/main.py`.** Currently only #11-#15 are wired; **all five remaining tools** (#16 goal pace, #17 propose intervention, #18 log outcome, #19 schedule reminder, #20 summarize week) are callable but unwired. Add tool descriptions to `_DESCRIPTIONS` + wrappers to `_build_tools` in a single pass. The `_DESCRIPTIONS` dict ALREADY references `summarize_week` from inside `query_transactions`' description — that forward reference becomes truthful the moment this lands. | @mahyar | S | **THE bottleneck of Sprint 2.** Sprint 2 tool build-out is complete (10/10 tools, 196 tests); the agent CANNOT call any of the new five in chat until this runs. **MUST run before slide-8 demo recording.** |
