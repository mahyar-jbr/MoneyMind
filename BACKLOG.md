...
| # | Item | Owner | Size |
| --- | ---- | ----- | ---- |
| R2 | ✅ **done 2026-06-05** — Vertex AI migration verified live. `'hello from vertex'` returned in 4.9s via `moneymind-agent@moneymind-hack.iam` SA. 222 tests pass, ruff clean. Import fix: `langchain_google_vertexai.chat_models` submodule (cold-import 236s → 2.1s). GCP: project `moneymind-hack`, role `roles/aiplatform.user`, key at `.gcloud/service-account.json` (gitignored), billing on free trial. | @mahyar | — |
