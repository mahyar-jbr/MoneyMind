...
| 23a | ⬜ todo | **Dashboard must render `/inbox`** — Kasra's #21 ships JSON at GET `/inbox` with weekly-summary + reminder messages. Aidin's #23 dashboard merged WITHOUT rendering it. The demo video's "agent posts to inbox" beat depends on Aidin surfacing it visibly. | @aidin | S | Re-flagged by #23 review. Without this, `/inbox` is invisible product. |
