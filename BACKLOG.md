...
| 12a | ⚠️ **10 reports flagged (counter capped)** | **Delete `backend/app/api/chat 2.py` + add `* 2.*` patterns to `.gitignore`.** Byte-identical Finder duplicate, still on disk. #18 dev report: "I'm out of hyperbole." Mahyar: tell Kasra he can just bundle the `git rm` into whatever backend PR he ships next. | @kasra | S | Counter no longer tracked — flagged in 10 consecutive dev reports. |
...
| 17-wire | ⬜ todo | **Append #16 + #17 + #18 (and #19 + #20 once landed) to `_build_tools` in `agent/graphs/main.py`.** Currently only #11-#15 are wired into the LangGraph graph; #16, #17, #18 are all stacked unwired (and #19 + #20 will land before this fires). Add tool descriptions to `_DESCRIPTIONS` and wrappers to `_build_tools`. Run AFTER #20 lands so all 10 wire in one pass. | @mahyar | S | The "callable but unwired" convention scales to ~2-3 tickets; with 5+ stacked it's a real gap. MUST run before slide-8 demo recording — agent can't call these tools in chat until it does. |
